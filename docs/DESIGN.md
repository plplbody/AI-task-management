...
*   **`TaskList.tsx` (プレゼンテーショナルコンポーネント)**
    *   **責務**: `App.tsx`から受け取ったタスクとヘッダーのデータをレイアウトし、テーブルとして表示することに専念します。
    *   **役割**: 自身ではAPI通信を行いません。ユーザーのアクション（ボタンクリックなど）が発生すると、propsで渡された`App.tsx`の関数を呼び出すだけです。ヘッダーラベルのインライン編集に関する一時的なUI状態のみ、自身のstateで管理します。

*   **`TaskItem.tsx` (プレゼンテーショナルコンポーネント)**
...
クリーンで直感的な操作性を実現するため、以下の配色を基本方針とします。

*   **基本背景色**: `#FFFFFF` (白)
*   **基本文字色**: `#5F5F5F` (濃いグレー)
*   **ベースカラー**: 青 (`#4472C4`), 緑 (`#70AD47`), オレンジ (`#FFC000`) の3色を基調とします。
*   **役割ごとの配色**:
    *   **主要な見出し・テーブルヘッダー (青)**: 全体の構造を明確化します。
    *   **追加処理 (緑)**: 新規作成などのポジティブなアクションを示します。
    *   **削除・注意喚起 (オレンジ)**: 削除などの注意を要するアクションを示します。
    *   **文字色**: 背景が濃い色（青、緑、オレンジ）の場合は、文字色を白にし、可読性を確保します。
