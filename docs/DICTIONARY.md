...
- **`express`**: Node.jsでWebサーバーを簡単に構築するためのフレームワークです。APIの土台として機能します。
  ...
  **使用例 (`server.js`):**
  ...
  #### 非同期処理との連携 (`async/await`)
  Expressでデータベースアクセスなど時間のかかる処理（非同期処理）を行う場合、`async/await`を一緒に使うのが一般的です。これにより、処理が終わるのを待ってからレスポンスを返す、という流れを簡単に記述できます。

  - **`async`**: `(req, res)`を受け取るコールバック関数の前に置くことで、その関数内で`await`キーワードが使えるようになります。
  - **`await`**: データベースへの問い合わせなど、時間のかかる処理の前に置きます。これにより、その処理が完了するまで後続のコードの実行を待ちます。
  - **`try...catch`**: `await`を使った処理でエラーが起きた場合に備え、`try...catch`で囲むのが定石です。

  **`async/await`を使った使用例 (`server.js`):**
  ```javascript
  // 1. `async`を関数の前に追加
  app.get('/api/tasks', async (req, res) => {
    try {
      // 2. DBからの応答を`await`で待つ
      const result = await pool.query('SELECT * FROM tasks');
      
      // 3. 応答が返ってきた後で、レスポンスを返す
      res.json(result.rows);
    } catch (err) {
      // 4. もしエラーが起きたら、エラーレスポンスを返す
      console.error(err);
...
- **`cors`**: フロントエンドとバックエンドが異なるドメイン（この場合は`localhost:5173`と`localhost:3001`）で動作している場合に、ブラウザのセキュリティ制限（CORSポリシー）を回避し、通信を許可するためのミドルウェアです。
  ...
  **使用例 (`server.js`):**
  ...
  CORSを理解するために、ブラウザを「セキュリティが厳しいマンション」だと想像してみてください。

  1.  **基本ルール（同一オリジンポリシー）**:
      -   このマンションの基本ルールは、「**マンション内部からの連絡しか受け取ってはいけない**」というものです。これは、悪意のある外部の人間が勝手に部屋の中の情報を盗めないようにするための、重要なセキュリティルールです。
      -   Webの世界では、この「同じマンション」のことを「**同じオリジン**（同じドメイン、同じポート番号）」と呼びます。

  2.  **発生する問題**:
      -   今回のアプリでは、フロントエンドは「`5173`号室」、バックエンドは「`3001`号室」に住んでいます。
      -   フロントエンド（`5173`号室）が、データをくださいとバックエンド（`3001`号室）に電話をかけようとします。
      -   しかし、マンションのセキュリティシステム（ブラウザ）が、「待ってください！ `5173`号室から`3001`号室への連絡は、ルール違反です（オリジンが違うため）。安全のため通信をブロックします」と警告し、通信を強制的に切断してしまいます。これが「**CORSエラー**」の正体です。

  3.  **解決策 (CORS)**:
      -   そこで、バックエンド（`3001`号室）側が、あらかじめマンションの受付（ブラウザ）に「**`5173`号室からの連絡は、知り合いなので許可してください**」という届け出を出しておきます。
      -   この「許可の届け出」の仕組みが **CORS (Cross-Origin Resource Sharing / オリジン間リソース共有)** です。
      -   `app.use(cors())` というコードは、この届け出をバックエンドサーバーが自動的に行ってくれるように設定する、という意味になります。

  この設定により、ブラウザは安心してフロントエンドとバックエンドの通信を許可してくれるようになります。
